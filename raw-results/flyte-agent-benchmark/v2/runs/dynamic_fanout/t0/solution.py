...
@env.task
async def compute_n(seed: int) -> int:
    return (seed % 5) + 3
...
@env.task
async def main(seed: int) -> dict[str, int]:
    n = await compute_n(seed)
...
if __name__ == "__main__":
    ...
    print("TRIAL_OUTPUT_JSON:" + json.dumps({"n": result["n"], "total": result["total"]}))
